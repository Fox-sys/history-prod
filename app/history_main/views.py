...
from .models import MainUser, SolderPost, Exhibit, OutDoorSchool, Teacher
...
class ChangePasswordView(FormView):
    ...
    def post(self, request):
        ...
        return render(request, self.template_name_change_password, context={"form": self.form_class_change_password(),
                                                                            "message": "Неверный ключ или пароли не совпадают"})
...
class OutDoorSchoolList(ListPagerView):
    model = OutDoorSchool
    template_name = 'history_main/out_door_school.html'
    url = '/out_door_school/'
...
class OutDoorSchoolDetail(DetailView):
    model = OutDoorSchool
    template_name = 'history_main/out_door_school_detail.html'


class TeacherList(ListPagerView):
    model = Teacher
    template_name = 'history_main/teacher_list.html'
    url = '/teachers/'
    ...
    def names_search(self, queryset, search_info):
        ...
        if form.is_valid():
            result = self.model.objects.none()
            for i in form.get_names():
                result = result | queryset.filter(full_name__icontains=i)
        return result
